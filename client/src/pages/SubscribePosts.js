...
import { config as axiosConfig, SUB_POST_URL } from "../hooks/constants";
...
const SubscribePost = () => {
	...
	const { state } = useContext(AuthenticationContext);//switch between different tabs

	const [data, setData] = useState([]);//Read the value of data and setData
	const [showSend, setShowSend] = useState(false);//value of classes used to style
	const [comment, setComment] = useState(""); /***read the value of comment and setComment from context***/

	/**
      *configuration for Axios Library
    */
    ...
    // useEffect(() => LikePost, []);
	//creating like-post function from axios using ref id
	const likePost = (id) => {
		// put request like using axios
		axios.put(`http://localhost:5000/like`, { postId: id }, config)
			.then((result) => {
				const newData = data.map((item) => {
					//save newData to map item
					if (result.data._id === item._id) return result.data;
					else return item;
				});
				setData(newData);// collect new likepost
			})
			.catch((err) => console.log(err)); //handle errors
	};
// useEffect(() => UnlikePost, []);
	const UnlikePost = (id) => {
		axios.put(`http://localhost:5000/Unlike`, { postId: id }, config)
			.then((res) => {
				...
				setData(newData);//save new input data
			})
			.catch((err) => console.log(err)); //handle errors
	};
	// useEffect(() => makeComment, []);
	const makeComment = (text, postId) => {
		setComment("");
		axios.put(`http://localhost:5000/comment`, { text, postId }, config)
			.then((result) => {//api response
				const newData = data.map((item) => {
					//input newData to map item
					if (result.data._id === item._id) return result.data;
					else return item;
				});
				setData(newData);//save
			})
			.catch((err) => console.log(err));//handle errors
	};
	...
	const deletePost = (postId) => {
		axios.delete(`http://localhost:5000/deletepost/${postId}`, config).then((res) => {
			const newData = data.filter((item) => {
				//input data filter to item
				return item._id !== res.data;// requested postId
				/**response data**/
			});
			setData(newData);//save new data
		});
	};
