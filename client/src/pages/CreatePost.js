...
import { config as axiosConfig, CREATE_POST_URL } from "../hooks/constants";
// import Material-UI deps
...
//import FilePond deps
// Importing named values:
/*https://formvalidation.io/guide/api/register-plugin/ */
...
/* This constructor defines a created post object with properties */
...
const CreatePoste = () => {
	...
	const history = useHistory();// Dictionary used to save a reference [state, setState] for each input
	const [files, setFiles] = useState([]);//read the value of Files
	const [caption, setCaption] = useState("");//read the value of setCaption from context
	const [activeStep, setActiveStep] = useState(0);//read the value of activestep from context
	...
	const PostData = () => {
		...
		Axios.post(
			...
			if (rep.data.message) {//response message
				setQuery("success");//post successful
			}
		});
	};
  // useEffect(() => getStepContent, []); You don't need two effects that does the same task
...
// Can call any methods on this ref
	const handleBack = () => {
		setActiveStep((prevActiveStep) => prevActiveStep - 1);
	};
