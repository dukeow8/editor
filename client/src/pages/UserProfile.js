...
import { config as axiosConfig } from "../hooks/constants";
...
const UserProfilePage = () => {
	const classes = useStyles();//value of classes 
	const [value, setValue] = useState("Posts"); // to switch between different tabs
	const { state, dispatch } = useContext(AuthenticationContext);
	const { userid } = useParams();//value of user from useParams
    ...
    */
	...
	useEffect(() => {
		axios.get(`http://localhost:5000/user/${userid}`, config).then((res) => {
			setData(res.data);//response data from userId
		});
	}, []);// eslint-disable-line react-hooks/exhaustive-deps

	// useEffect(() => followUser, []);
	const followUser = () => {
					...
					<Box mb="44px">
						<Grid container>
							...
							<Grid item xs={8}>
								...
								<Typography variant="subtitle1">Ndumiso Banda</Typography>
								<Typography variant="subtitle1">Web developer</Typography>
								<Typography variant="subtitle1">Nelspruit 1203.</Typography>
							</Grid>
						</Grid>
					</Box>
					<Tabs
