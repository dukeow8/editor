...
import { RESET_PWD_URL } from "../hooks/constants";
...
const Reset = () => {
	...
	const [emailCheck, setEmailCheck] = useState(false);/*read the value of emailcheck and setEmailcheck */
	const [errorMsg, setErrorMsg] = useState(false);/***read the value of errorMsg***/
	const [successMsg, setSuccessMsg] = useState(false);/***read the value of successMsg***/
    //timerRef.current.start(); <-- function Stopwatch
	const timerRef = useRef();//save timer in useRef and pass it to child
	// useRef value stored 
	useEffect(
				...
				.then((res) => {
					...
					if (data.error) {
						...
						timerRef.current = setTimeout(() => {
							history.push("/login");
						}, 3000);//wait for atleast  3 seconds before login
					}
				})
				.catch((err) => {
					console.log(err);//handle response error
				});
		} else {
			setErrorMsg(false);
			setEmailCheck(true);
